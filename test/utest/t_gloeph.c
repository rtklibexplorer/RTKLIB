...
static void dumpgeph(geph_t *geph, int n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    printf(
        ...
        i + 1, geph[i].sat, geph[i].frq, geph[i].svh, geph[i].age, s1, s2, geph[i].pos[0],
        geph[i].pos[1], geph[i].pos[2], geph[i].vel[0], geph[i].vel[1], geph[i].vel[2],
        geph[i].acc[0], geph[i].acc[1], geph[i].acc[2], geph[i].taun, geph[i].gamn);
  }
}
/* readrnx() */
void utest1(void) {
