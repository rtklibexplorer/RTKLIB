...
static void dumpobs(obs_t *obs) {
  ...
  for (i = 0; i < obs->n; i++) {
    ...
    printf("%s : %2d %2d %2s: %13.3f %13.3f %d, %2s: %13.3f %13.3f %d, %2s: %13.3f %13.3f %d\n",
           str, obs->data[i].sat, obs->data[i].rcv, code2obs(obs->data[i].code[0]),
           obs->data[i].L[0], obs->data[i].P[0], obs->data[i].LLI[0],
           code2obs(obs->data[i].code[1]), obs->data[i].L[1], obs->data[i].P[1],
           obs->data[i].LLI[1], code2obs(obs->data[i].code[2]), obs->data[i].L[2],
           obs->data[i].P[2], obs->data[i].LLI[2]);
...
static void dumpnav(nav_t *nav) {
  ...
  for (i = 0; i < nav->n; i++) {
    ...
    printf("%s : %2d    %s %s %3d %3d %2d\n", str, nav->eph[i].sat, s1, s2, nav->eph[i].iode,
