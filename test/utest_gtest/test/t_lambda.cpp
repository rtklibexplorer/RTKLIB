...
TEST(TEST_LAMBDA, test_1)
{
    ...
    for (j=0;j<m;j++) {
        for (i=0;i<n;i++) {
            EXPECT_TRUE(fabs(F[i+j*n]-F1[j+i*m])<1E-4);
        }
        EXPECT_TRUE(fabs(s[j]-s1[j])<1E-4);
    }
}
TEST(TEST_LAMBDA, test_2)
{
    int i,j,n,m,info;
    double F[10*2],s[2];
    ...
    for (j=0;j<m;j++) {
        for (i=0;i<n;i++) {
            EXPECT_TRUE(fabs(F[i+j*n]-F2[j+i*m])<1E-4);
        }
        EXPECT_TRUE(fabs(s[j]-s2[j])<1E-4);
    }
}
