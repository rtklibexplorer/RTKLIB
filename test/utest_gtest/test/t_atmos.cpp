...
TEST(Testionmodel,utest1)
{
    ...
    EXPECT_TRUE(fabs(dion-6.73590532099438)<1e-8);
    
    dion=ionmodel(t1,ion1,pos2,azel1);
    EXPECT_TRUE(fabs(dion-3.56895382197387)<1e-8);
    
    dion=ionmodel(t1,ion1,pos3,azel1);
    EXPECT_TRUE(fabs(dion-3.80716435655161)<1e-8);
    
    dion=ionmodel(t2,ion1,pos1,azel1);
    EXPECT_TRUE(fabs(dion-5.21796954585452)<1e-8);
    
    dion=ionmodel(t3,ion1,pos1,azel1);
    EXPECT_TRUE(fabs(dion-5.90190539264777)<1e-8);
    
    dion=ionmodel(t1,ion1,pos1,azel2);
    EXPECT_TRUE(fabs(dion-21.6345415123632)<1e-8);
    
    dion=ionmodel(t1,ion1,pos1,azel3);
    EXPECT_TRUE(fabs(dion-7.33844278822561)<1e-8);
    
    dion=ionmodel(t1,ion1,pos1,azel4);
    EXPECT_TRUE(fabs(dion-6.58339711400694)<1e-8);
    
    dion=ionmodel(t1,ion2,pos1,azel1);
    EXPECT_TRUE(fabs(dion-6.73590532099438)<1e-8);
}
/* ionmapf() */
TEST(Testionmapf,utest2)
{
}
/* tropmodel */
TEST(Testtropmodel, utest3)
{
    gtime_t time={0};
    double pos1 []={ 35*D2R, 140*D2R, 100.0};
    double pos2 []={-80*D2R,-170*D2R,1000.0};
    ...
    EXPECT_TRUE(fabs(dtrp-2.44799870144088)<1e-8);
    
    dtrp=tropmodel(time,pos1,azel2,0.5);
    EXPECT_TRUE(fabs(dtrp-45.1808916506163)<1e-8);
    
    dtrp=tropmodel(time,pos2,azel1,0.5);
    EXPECT_TRUE(fabs(dtrp-2.17295817298152)<1e-8);
    
    dtrp=tropmodel(time,pos1,azel3,0.0);
    EXPECT_TRUE(fabs(dtrp-0.00000000000000)<1e-8);
...
TEST(Testtropmodel, utest4)
{
    ...
    EXPECT_TRUE(fabs(mapfd-1.035184526466435)<1e-8);
    EXPECT_TRUE(fabs(mapfw-1.035233787448654)<1e-8);
    
    mapfd=tropmapf(t1,pos1,azel2,&mapfw);
    EXPECT_TRUE(fabs(mapfd-14.643271711748200)<1e-8);
    EXPECT_TRUE(fabs(mapfw-16.455045694559484)<1e-8);
    
    mapfd=tropmapf(t1,pos1,azel3,&mapfw);
    EXPECT_TRUE(fabs(mapfd-1.154226397147367)<1e-8);
    EXPECT_TRUE(fabs(mapfw-1.154481126139610)<1e-8);
    
    mapfd=tropmapf(t1,pos1,azel4,&mapfw);
    EXPECT_TRUE(fabs(mapfd-1.000000000000000)<1e-8);
    EXPECT_TRUE(fabs(mapfw-1.000000000000000)<1e-8);
    
    mapfd=tropmapf(t2,pos1,azel1,&mapfw);
    EXPECT_TRUE(fabs(mapfd-1.035184415128022)<1e-8);
    EXPECT_TRUE(fabs(mapfw-1.035233787448654)<1e-8);
    
    mapfd=tropmapf(t1,pos2,azel1,&mapfw);
    EXPECT_TRUE(fabs(mapfd-1.035186155749051)<1e-8);
    EXPECT_TRUE(fabs(mapfw-1.035230548304367)<1e-8);
    
    mapfd=tropmapf(t1,pos3,azel1,&mapfw);
    EXPECT_TRUE(fabs(mapfd-1.035181919429758)<1e-8);
    EXPECT_TRUE(fabs(mapfw-1.035233200318210)<1e-8);
