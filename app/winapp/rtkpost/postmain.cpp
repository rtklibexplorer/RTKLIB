...
void __fastcall TMainForm::LoadOpt(void) {
  ...
  for (int i = 0; i < NFREQ; i++)
    for (int j = 0; j < 9; j++) {
      SnrMask.mask[i][j] = ini->ReadFloat("opt", s.sprintf("snrmask_%d_%d", i + 1, j + 1), 35.0);
    }
...
void __fastcall TMainForm::SaveOpt(void) {
  ...
  ini->WriteInteger("conv", "format",
                    ConvDialog->FormatKML->Checked ? 0 : (ConvDialog->FormatCSV->Checked ? 1 : 2));
...
void __fastcall TMainForm::Panel4Resize(TObject *Sender) {
  TButton *btns[] = {BtnInputFile1, BtnInputFile2, BtnInputFile3, BtnInputFile4, BtnInputFile5,
                     BtnInputFile6, BtnInputFile7, BtnInputFile8, BtnInputFile9, BtnInputFile10};
  TComboBox *boxes[] = {InputFile1, InputFile2, InputFile3, InputFile4, InputFile5,
