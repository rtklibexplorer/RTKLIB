...
//---------------------------------------------------------------------------
void __fastcall TConvDialog::SingleMeanFClick(TObject *Sender) { UpdateEnable(); }
//---------------------------------------------------------------------------
...
void __fastcall TConvDialog::BtnConvertClick(TObject *Sender) {
  ...
  if (FormatKML->Checked) {
    ...
    stat = convkml(file, Compress->Checked ? kmlfile : OutputFile_Text.c_str(), ts, te, tint,
                   QFlags->ItemIndex, mean, name, offset, TrackColor->ItemIndex,
                   PointColor->ItemIndex, OutputAlt->ItemIndex, OutputTime->ItemIndex);
  } else if (FormatCSV->Checked) {
    stat = convcsv(file, Compress->Checked ? kmlfile : OutputFile_Text.c_str(), ts, te, tint,
                   QFlags->ItemIndex, mean, name, offset, OutputAlt->ItemIndex,
                   OutputTime->ItemIndex, csvorder);
  } else {
...
void __fastcall TConvDialog::UpdateOutFile(void) {
  ...
  strcpy(p, FormatGPX->Checked
                ? ".gpx"
                : (FormatCSV->Checked ? ".csv" : (Compress->Checked ? ".kmz" : ".kml")));
