...
void __fastcall TMonitorDialog::ShowRtk(void) {
  ...
  *rtk = rtksvr.rtk;
  ...
  Tbl->Cells[1][i++] = !rtk->opt.snrmask.ena[0]
                           ? s.sprintf("")
                           : s.sprintf("%.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f",
                                       rtk->opt.snrmask.mask[3][0], rtk->opt.snrmask.mask[3][1],
                                       rtk->opt.snrmask.mask[3][2], rtk->opt.snrmask.mask[3][3],
                                       rtk->opt.snrmask.mask[3][4], rtk->opt.snrmask.mask[3][5],
                                       rtk->opt.snrmask.mask[3][6], rtk->opt.snrmask.mask[3][7],
                                       rtk->opt.snrmask.mask[3][8]);
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.3f, %.3f, %.3f, %.3f", rtk->sol.dtr[1] * 1E9, rtk->sol.dtr[2] * 1E9,
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.3f, %.3f, %.3f", rtk->sol.rr[0], rtk->sol.rr[1], rtk->sol.rr[2]);
  ...
  Tbl->Cells[1][i++] = s.sprintf("%.3f, %.3f, %.3f", rtk->xa ? rtk->xa[0] : 0,
...
void __fastcall TMonitorDialog::ShowObs(void) {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (k = 0; k < NFREQ + nex; k++) {
      if (obs[i].SNR[k])
        Tbl->Cells[j++][i + 1] = s.sprintf("%.1f", obs[i].SNR[k]);
      else
...
void __fastcall TMonitorDialog::ShowSbsNav(void) {
  ...
  for (i = 0, n = 1; i < NSATSBS; i++) {
    valid = fabs(timediff(time, seph[i].t0)) <= MAXDTOE_SBS && seph[i].t0.time && seph[i].svh == 0;
    if (SelSat->ItemIndex == 1 && !valid) continue;
    n++;
  }
  if (n < 2) {
...
void __fastcall TMonitorDialog::ShowStr(void) {
  ...
  AnsiString outformat[] = {"Lat/Lon/Height", "X/Y/Z-ECEF",     "E/N/U-Baseline",
                            "NMEA-0183",      "Solution stats", "GSI F1/F2"};
