...
void __fastcall TMainForm::SvrStart(void) {
  ...
  if ((RovAntPcvF || RefAntPcvF) && AntPcvFileF != "" &&
      !readpcv(AntPcvFileF.c_str(), &rtksvr.pcvsr)) {
...
void __fastcall TMainForm::DrawTrk(TImage *plot) {
  ...
  TPoint p1, p2;
  UTF8String label;
  double scale[] = {0.000021, 0.000047, 0.0001, 0.00021, 0.00047, 0.001,  0.0021, 0.0047, 0.01,
                    0.021,    0.047,    0.1,    0.21,    0.47,    1.0,    2.1,    4.7,    10.0,
                    21.0,     47.0,     100.0,  210.0,   470.0,   1000.0, 2100.0, 4700.0, 10000.0};
  ...
  for (i = 0; i < MAXSAT * 2; i++) {
    ...
    *p = '\0';
    ...
    sscanf(p + 1,
           ...
           &nav->eph[i].iode, &nav->eph[i].iodc, &nav->eph[i].sva, &nav->eph[i].svh, &toe_time,
           &toc_time, &ttr_time, &nav->eph[i].A, &nav->eph[i].e, &nav->eph[i].i0, &nav->eph[i].OMG0,
           ...
           &nav->eph[i].fit, &nav->eph[i].f0, &nav->eph[i].f1, &nav->eph[i].f2, &nav->eph[i].tgd[0],
           &nav->eph[i].code, &nav->eph[i].flag, &nav->eph[i].tgd[1]);
...
void __fastcall TMainForm::LoadOpt(void) {
  ...
  PrcOpt.navsys =
      ini->ReadInteger("prcopt", "navsys", SYS_GPS | SYS_GLO | SYS_GAL | SYS_QZS | SYS_CMP);
