...
static int simobs(simopt_t simopt, rnxopt_t rnxopt, nav_t *nav, obs_t *obs) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (simopt.tidecorr > 0) {
      dr[0] = dr[1] = dr[2] = 0.0;
      tidedisp(gpst2utc(time), simopt.rr, simopt.tidecorr, &nav->erp, simopt.odisp, dr);
    ...
    for (j = 0; j < MAXSAT; j++) {
      ...
      if (satpos(time, time, data[j].sat, EPHOPT_PREC, nav, rs, dts, &var, &svh) == 0) continue;
      ...
      for (k = 0; k < rnxopt.nobs[iSys]; k++) {
        ...
        cp = r + CLIGHT * (dtr - dts[0]) - (simopt.ionoopt > 0 ? fact * iono : 0.0) +
             (simopt.tropopt > 0 ? trop : 0.0) + (simopt.noise > 0 ? epr : 0.0);
        pr = r + CLIGHT * (dtr - dts[0]) + (simopt.ionoopt > 0 ? fact * iono : 0.0) +
             (simopt.tropopt > 0 ? trop : 0.0) + (simopt.noise > 0 ? ecp : 0.0);
...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-o") && i + 1 < argc) {
      ...
      if (nc < MAXCOMMENT) strcpy(rnxopt.comment[nc++], argv[++i]);
    ...
    /* Signal mask */
      ...
      for (j = 0; j < 7; j++)
        for (k = 0; k < 64; k++) rnxopt.mask[j][k] = '0';
