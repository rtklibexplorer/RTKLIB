...
static void printssrmsg(int sat, const ssr_t *ssr, int topt, int mopt) {
  ...
  if (mopt & 2) {
    printf("%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f ", ssr->deph[0], ssr->deph[1], ssr->deph[2],
           ssr->ddeph[0], ssr->ddeph[1], ssr->ddeph[2]);
  }
  if (mopt & 4) {
    printf("%8.3f %8.3f %8.3f %8.3f ", ssr->dclk[0], ssr->dclk[1], ssr->dclk[2], ssr->hrclk);
  }
  if (mopt & 8) {
    printf("%6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f ",
           ssr->cbias[0], ssr->cbias[1], ssr->cbias[2], ssr->cbias[3], ssr->cbias[4], ssr->cbias[5],
           ssr->cbias[6], ssr->cbias[7], ssr->cbias[8], ssr->cbias[9], ssr->cbias[10],
           ssr->cbias[11]);
  }
  printf("\n");
}
