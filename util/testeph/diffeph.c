...
/* print difference ----------------------------------------------------------*/
static void printephdiff(gtime_t time, int sat, int eph1, int eph2, const nav_t *nav1,
                         const nav_t *nav2, int topt, int eopt, int mopt, const double *pos) {
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < (int)(tspan * 3600.0 / tint); i++) {
    ...
    for (s = 1; s <= MAXSAT; s++) {
      ...
      printephdiff(time, s, eph1, eph2, &nav, eph1 == 1 && eph2 == 1 ? &nav2 : &nav, topt, eopt,
                   mopt, pos);
    }
  }
  if (fp) fclose(fp);
