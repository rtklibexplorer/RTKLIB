...
static void out_iono(gtime_t time, int sat, int brk, double iono, double rate, double var,
                     FILE *fp) {
  double tow;
  char id[8];
  int week;
  ...
  fprintf(fp, "%s %4d %6.0f %-3s %d %8.4f %9.6f %7.4f\n", STEC_RID, week, tow, id, brk, iono, rate,
          var);
}
/* interpolation of stec to grid ---------------------------------------------*/
static void interp_grid(const nav_t *nav, gtime_t ts, gtime_t te, double tint, const double *pos,
                        const char *dir) {
  FILE *fp = NULL;
  gtime_t t0 = {0}, t1;
  ...
  for (i = 0;; i++, t0 = t1) {
    ...
    for (sat = 1; sat <= MAXSAT; sat++) {
      ...
      if (!interp_ion(nav, t0, t1, sat, n, index, dist, iono + sat - 1, &rate, &var, &brk)) {
        continue;
      }
      if (!fp) {
        sprintf(file, "%s/%c%4.0f_%c%5.0f.stec", dir, pos[0] < 0.0 ? 'S' : 'N',
                fabs(pos[0]) * R2D * 100.0, pos[1] < 0.0 ? 'W' : 'E', fabs(pos[1]) * R2D * 100.0);
...
static void interp_stec(const nav_t *nav, gtime_t ts, gtime_t te, double tint, const double *ew,
                        const double *ns, double gint, const char *dir) {
