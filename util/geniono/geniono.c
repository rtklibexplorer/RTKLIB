...
static int raw_obs(const obsd_t *obs, const nav_t *nav, double *P1, double *P2, double *L1,
                   double *L2) {
...
static void ud_state(const obsd_t *obs, int n, const nav_t *nav, const double *pos,
                     const double *azel, ekf_t *ekf, sstat_t *sstat) {
...
static int res_iono(const obsd_t *obs, int n, const nav_t *nav, const double *rs, const double *rr,
                    const double *pos, const double *azel, const pcv_t *pcv, const ekf_t *ekf,
...
static void out_iono(gtime_t time, const ekf_t *ekf, const sstat_t *sstat, FILE *fp) {
  double tow;
  char id[8];
  int sat, week;
  ...
  for (sat = 1; sat <= MAXSAT; sat++) {
    if (sstat[sat - 1].time.time == 0 || timediff(time, sstat[sat - 1].time) > MAXGAP_IONO)
      continue;
    satno2id(sat, id);
    fprintf(fp, "%s %4d %6.0f %-3s %d %8.4f %9.6f %7.4f %6.1f %5.1f %7.3f %11.3f\n", STEC_RID, week,
            tow, id, sstat[sat - 1].slip, ekf->x[II(sat)], ekf->x[II(sat) + 1],
            sqrt(ekf->P[II(sat) * (ekf->nx + 1)]), sstat[sat - 1].azel[0] * R2D,
            sstat[sat - 1].azel[1] * R2D, sstat[sat - 1].PG, sstat[sat - 1].LG);
  }
}
/* estimate ionosphere -------------------------------------------------------*/
static int est_iono(obs_t *obs, nav_t *nav, const pcv_t *pcv, double *rr, double tint, FILE *fp) {
