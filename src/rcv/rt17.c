...
EXPORT int input_rt17(raw_t *Raw, uint8_t Data) {
  ...
  */
  ...
  if (Raw->outtype) sprintf(Raw->msgtype, "RT17 0x%02X (%4u)", PacketBuffer[2], rt17->PacketLength);
  ...
  if (PacketBuffer[2] == GENOUT) {
    ...
    */
    if (rt17->MessageBytes == 0) {
      ...
      tracet(2,
             "RT17: GENOUT packet sequence number mismatch or page out of order. %u GENOUT packets "
             "discarded.\n",
             Page);
      ClearMessageBuffer(rt17);
      ClearPacketBuffer(rt17);
  ...
  *rif = p_in[7];
  ...
  while (InputLengthTotal > 0) {
    if ((uint32_t)p_in[7] != *rif)
      tracet(2,
             "RT17: Inconsistent Record Interpretation Flags within a single RAWDATA message.\n");
