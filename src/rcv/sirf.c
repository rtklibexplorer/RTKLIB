...
static float R4U(unsigned char *p) {float          r; memcpy(&r,p,4); return r;}
...
static int decode_sirfclock(raw_t *raw)
{
    ...
    /* NOTE Delta TOW -> Zeitl. Abstand zw. zwei Messungen*/
    if (lastTOW==-1) lastTOW=U4(p+3);
    /* printf("\ntd=%d[Hz]/1575.42[MHz]=%f[s]",drift,(float)drift/FREQL1); printf("\ndtb=%d-%d=%d[ns] ",U4(p+12),lastClkB,U4(p+12)-lastClkB); */
    /*  printf("\n(4) (td+dtb)*c=(%f[s]+%f[s])*%.0f[m/s]=%.3f[m]",(float)drift/FREQL1,(float)(U4(p+12)-lastClkB)/1e+9,CLIGHT,(float)CLIGHT*(drift/FREQL1+(U4(p+12)-lastClkB)/1e+9));*/
    if (U4(p+3)>lastTOW) {
        dtb=U4(p+3)-lastTOW; dtc=(U4(p+12)-lastClkB)/1e+9;
        /* printf("DeltaTOW[%d]=%.2f",Cnt,(float)dtb/100); */
        Cnt=0; lastTOW=U4(p+3); lastClkD=U4(p+8); lastClkB=U4(p+12); 
    } else Cnt++;
    ...
    for (i=0;i<raw->obuf.n&&i<MAXOBS;i++) {
        ...
        if (dst->L[0]) {
            ...
            if ((auxData[dst->sat].extStat&0x3c)>>2) {
                dst->LLI[0]=1;							/* Cycle Slip, Subframe sync, Multipath */
                printf("\nSAT#%2d: Cycle Slip detected ...",dst->sat);
            }
            if ((auxData[dst->sat].RecovStat&0x14)>>2) {
                dst->LLI[0]=1;							/* Bad PrePos, wrong BitSync		*/
                printf("\nSAT#%2d: Bad PrePos or Sync ...",dst->sat);
            }
        ...
        /* dst->D[0]=0; ... keine Auswirkung! */
            ...
            printf("PseudoRng[dT]=%.0f ",dst->L[0]/dst->D[0]*CLIGHT/1000);
            tor=U4(p+3)/100-bias; tot=U4(p+3)/100-(dst->L[0]/(dst->D[0]*CLIGHT))*CLIGHT;
            printf("dt=%.3f->PseudoR[GPS]=%.0f[km] ",tor-tot,CLIGHT*(tor-tot)/1000);
            dst->P[0]=dst->P[0];							/* Use Receiver Data, "smoothed" by Carrier Phase	*/
            dst->P[0]=dst->L[0]*CLIGHT/FREQL1;						/* Reconstruct from Carrier Phase			*/
            dst->P[0]=dst->L[0]/dst->D[0]*CLIGHT;					/* Reconstruct from Carrier Phase & Doppler Frequency	*/
...
/*    printf("DEC: "); for (i=0;i<=raw->len;i++) printf("%02X",U1(p+i)); */
/*    printf("\nXXX CF=%08X->%f->%f XXX",U4(p+23),R4(p+23),R4(p+23)*FREQL1/CLIGHT);  */
    obsd->L[0]=R8(p+27,gsw230)*FREQL1/CLIGHT;  /*((U1(p+37)&0x02)>>1);	/* Carrier Phase [m], report as Cycles					*/
/*    printf("\nDEBUG L=%.0f[m]->%0.f[cy] ",R8(p+27,gsw230),R8(p+27,gsw230)*FREQL1/CLIGHT);*/
    for (i=120;i>0;i--) {
        auxData[obsd->sat].cn[i+10]=auxData[obsd->sat].cn[i];
    }
...
/*    trace(4,"decode_sirfnlmeas: len=%d\n",raw->len);
    ...
    auxData[sat].clkdrift=R4(p+66);	/* R4, zum Beispiel passt aber S4 */
...
/*    printf("\nZZZ SAT=%2d RAW=%08X->%d",sat,U4(p+14),S4(p+14));
    ...
    printf("XXXXX CA=%04X->%d",U2(p+22),S2(p+22));
...
{
    int i,j,w,sid,vis;
    int marray[24] = {MID_SRFCLOCK, MID_SRF50BPS, MID_SRFNLMEAS, MID_SRFALM, MID_SRFEPH, 71, 2, 4, 9, 13, 130, 31, 41, 50, 51, 56, 164, 91, 92, 93, 11, 18, 225 };
    /* 130,141,164 193 */
    unsigned gsw230=strstr(raw->opt,"-GSW230")!=NULL;
