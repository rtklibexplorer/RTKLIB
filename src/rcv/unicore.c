...
static int sig2code(int sys, int sigtype, int l2c) {
  if (sys == SYS_GPS) {
    switch (sigtype) {
      ...
      case 9:
        return l2c == 1 ? CODE_L2S : CODE_L2W;  // L2P(Y),semi-codeless or L2C(M)
      case 3:
        return CODE_L1L;  // L1C Pilot
      case 11:
        return CODE_L1S;  // L1C Data, semi-codeless
      case 6:
        return CODE_L5I;  // L5 Data
      case 14:
        return CODE_L5Q;  // L5 Pilot
      case 17:
        return CODE_L2L;  // L2C(L)
      default:
        return 0;
    }
  } else if (sys == SYS_GLO) {
    ...
    switch (sigtype) {
      ...
      case 2:
        return CODE_L1C;  // E1C
      case 12:
        return CODE_L5Q;  // E5A Pilot
      case 17:
        return CODE_L7Q;  // E5B Pilot
      case 18:
        return CODE_L6B;  // E6B
      case 22:
        return CODE_L6C;  // E6C
    ...
    switch (sigtype) {
      ...
      case 4:
        return CODE_L2Q;  // B1Q
      case 8:
        return CODE_L1P;  // B1C pilot
      case 23:
        return CODE_L1D;  // B1C Data
      case 5:
        return CODE_L7Q;  // B2Q
      case 17:
        return CODE_L7I;  // B2I
      case 12:
        return CODE_L5P;  // B2a Pilot
      case 28:
        return CODE_L5D;  // B2a Data
      case 6:
        return CODE_L6Q;  // B3Q
      case 21:
        return CODE_L6I;  // B3I
      case 13:
        return CODE_L7P;  // B2b (I)
      default:
        return 0;
    }
  } else if (sys == SYS_IRN) {
    switch (sigtype) {
      case 6:
        return CODE_L5A;  // L5 Data
      case 14:
        return CODE_L5C;  // L5 Pilot
      default:
        return 0;
    }
  } else if (sys == SYS_SBS) {
...
static int decode_gpsephb(raw_t* raw) {
  ...
  if (!strstr(raw->opt, "-EPHALL")) {
    if (fabs(timediff(raw->nav.eph[sat - 1].toe, eph.toe)) < 1e-9 &&
        fabs(timediff(raw->nav.eph[sat - 1].toc, eph.toc)) < 1e-9)
      return 0;
  }
...
static int decode_gloephb(raw_t* raw) {
  ...
  week = U2(p);
  p += 2;
  tow = floor(U4(p) / 1000.0 + 0.5);
  p += 4; /* rounded to integer sec */
  toff = U4(p);
  p += 4;
  int Nt = U2(p);
  p += 2 + 2;
...
static int decode_unicore(raw_t* raw) {
  ...
  switch (type) {
    case ID_OBSVM:
      return decode_obsvmb(raw);
    case ID_GPSEPH:
      return decode_gpsephb(raw);
    case ID_GLOEPH:
      return decode_gloephb(raw);
    case ID_GALEPH:
      return decode_galephb(raw);
    case ID_BDSEPH:
      return decode_bdsephb(raw);
    case ID_QZSSEPH:
      return decode_qzssephb(raw);
    case ID_IRNSSEPH:
      return decode_irnssephb(raw);
  }
  return 0;
}
