...
static int decode_rxmrawx(raw_t *raw) {
  ...
  if (raw->outtype) {
    ...
    sprintf(raw->msgtype, "UBX RXM-RAWX  (%4d): time=%s nmeas=%d ver=%d", raw->len, tstr, nmeas,
            ver);
  }
  /* time tag adjustment option (-TADJ) */
  if ((q = strstr(raw->opt, "-TADJ="))) {
...
static int decode_rxmsfrbx(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(raw->msgtype, "UBX RXM-SFRBX (%4d): sys=%d prn=%3d sigid=%d", raw->len, U1(p),
            U1(p + 1), U1(p + 2));
  }
  if (!(sys = ubx_sys(U1(p)))) {
