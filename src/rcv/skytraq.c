...
static int decode_stqtime(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(raw->msgtype, "SKYTRAQ EPOCH (%4d): iod=%d week=%d tow=%.3f", raw->len, raw->iod, week,
            tow);
  }
  return 0;
}
...
static int decode_alm1(int sat, raw_t *raw) {
  ...
  if (sys == SYS_GPS) {
    decode_frame(raw->subfrm[sat - 1], sys, NULL, raw->nav.alm, raw->nav.ion_gps, raw->nav.utc_gps);
    adj_utcweek(raw->time, raw->nav.utc_gps);
  } else if (sys == SYS_QZS) {
    decode_frame(raw->subfrm[sat - 1], sys, NULL, raw->nav.alm, raw->nav.ion_qzs, raw->nav.utc_qzs);
...
static int decode_stqgps(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(raw->msgtype, "SKYTRAQ GPSSF (%4d): prn=%2d id=%d", raw->len, U1(p + 1),
            (p[8] >> 2) & 0x7);
  }
  prn = U1(p + 1);
...
static int decode_stqglo(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(raw->msgtype, "SKYTRAQ GLSTR (%4d): prn=%2d no=%d", raw->len, U1(p + 1) - 64,
            U1(p + 2));
  }
  prn = U1(p + 1) - 64;
...
static int decode_stqbds(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(raw->msgtype, "SKYTRAQ BDSSF (%4d): prn=%2d id=%d", raw->len, U1(p + 1) - 200,
            U1(p + 2));
  }
  prn = U1(p + 1) - 200;
