...
/* S/N ratio -----------------------------------------------------------------*/
static double snratio(double snr) { return snr <= 0.0 || 100.0 <= snr ? 0.0 : snr; }
...
static int decode_type1006(rtcm_t *rtcm) {
  ...
  if (rtcm->outtype) {
    ...
    for (j = 0; j < 3; j++) re[j] = rr[j] * 0.0001;
    ecef2pos(re, pos);
    sprintf(msg, " staid=%4d pos=%.8f %.8f %.3f anth=%.3f", staid, pos[0] * R2D, pos[1] * R2D,
            pos[2], anth * 0.0001);
  }
  /* test station id */
  if (!test_staid(rtcm, staid)) return -1;
...
static int decode_type1019(rtcm_t *rtcm) {
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg, " prn=%2d iode=%3d iodc=%3d week=%d toe=%6.0f toc=%6.0f svh=%02X", prn, eph.iode,
...
static int decode_type1020(rtcm_t *rtcm) {
  ...
  i += 5;
  int Cn = getbitu(rtcm->buff, i, 1);
  i += 1;
  int Cn_a = getbitu(rtcm->buff, i, 1);
  i += 1;
  int P1 = getbitu(rtcm->buff, i, 2);
  i += 2;
  ...
  i += 5;
  int P3 = getbitu(rtcm->buff, i, 1);
  i += 1;
  geph.gamn = getbitg(rtcm->buff, i, 11) * P2_40;
  i += 11;
  int P = getbitu(rtcm->buff, i, 2);
  i += 2;
  int ln = getbitu(rtcm->buff, i, 1);
  i += 1;
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg, " prn=%2d tk=%02.0f:%02.0f:%02.0f frq=%2d Bn=%d tb=%d", prn, tk_h, tk_m, tk_s,
            geph.frq, Bn, tb);
  }
  geph.sat = sat;
...
static int decode_type1041(rtcm_t *rtcm) {
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg, " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f svh=%02X", prn, eph.iode, week,
...
static int decode_type1044(rtcm_t *rtcm) {
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg, " prn=%3d iode=%3d iodc=%3d week=%d toe=%6.0f toc=%6.0f svh=%02X", prn, eph.iode,
            eph.iodc, week, eph.toes, toc, eph.svh);
  }
  if (!(sat = satno(sys, prn))) {
...
static int decode_type1045(rtcm_t *rtcm) {
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg, " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f hs=%d dvs=%d", prn, eph.iode, week,
...
static int decode_type1046(rtcm_t *rtcm) {
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg, " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f hs=%d %d dvs=%d %d", prn, eph.iode,
...
static int decode_type1042(rtcm_t *rtcm) {
  ...
  if (!strstr(rtcm->opt, "-EPHALL")) {
    if (fabs(timediff(eph.toe, rtcm->nav.eph[sat - 1].toe)) < 1.0 &&
        eph.iode == rtcm->nav.eph[sat - 1].iode && eph.iodc == rtcm->nav.eph[sat - 1].iodc)
      return 0; /* unchanged */
  }
  rtcm->nav.eph[sat - 1] = eph;
  rtcm->ephset = 0;
...
static int decode_msm_head(rtcm_t *rtcm, int sys, int *sync, int *iod, msm_h_t *h, int *hsize) {
  ...
  *hsize = i;
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg, " staid=%4d %s nsat=%2d nsig=%2d iod=%2d ncell=%2d sync=%d", staid, tstr, h->nsat,
...
static int decode_type1230(rtcm_t *rtcm) {
  ...
  if (rtcm->outtype) {
    sprintf(rtcm->msgtype + strlen(rtcm->msgtype), " staid=%4d align=%d mask=0x%X", staid, align,
            mask);
  }
  /* test station ID */
  if (!test_staid(rtcm, staid)) return -1;
