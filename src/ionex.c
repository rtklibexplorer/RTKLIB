...
static tec_t *addtec(const double *lats, const double *lons, const double *hgts, double rb,
  ...
  if (ndata[0] > 1 && ndata[1] > 1 && ndata[2] > 0) {
    ...
    for (i = 0; i < n; i++) {
      /* Thanks to 'if (ndata[0]>1 && ndata[1]>1 && ndata[2]>0)' we know analysis is wrong - disable
       * 6386 */
      p->data[i] = 0.0;
      p->rms[i] = 0.0f;
    }
...
/* read ionex body -----------------------------------------------------------*/
static int readionexb(FILE *fp, const double *lats, const double *lons, const double *hgts,
 ...
 *-----------------------------------------------------------------------------*/
extern int iontec(gtime_t time, const nav_t *nav, const double *pos, const double *azel, int opt,
                  double *delay, double *var) {
