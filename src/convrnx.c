...
static void setopt_file(int format, char **paths, int n, const int *mask, rnxopt_t *opt) {
  ...
  if (*opt->rcvopt) rnxcomment(opt, "options: %s", opt->rcvopt);
  for (int j = 0; j < n; j++) {
    if (!mask[j]) continue;
...
static int scan_file(char **files, int nf, rnxopt_t *opt, strfile_t *str, int *mask) {
  ...
  for (m = 0; m < nf && !abort; m++) {
    ...
    while ((type = input_strfile(str)) >= -1) {
      ...
      char tstr[40];
      snprintf(msg, sizeof(msg), "scanning: %s %s%s%s%s%s%s%s", time2str(str->time, tstr, 0),
               n[0] ? "G" : "", n[1] ? "R" : "", n[2] ? "E" : "", n[3] ? "J" : "", n[4] ? "S" : "",
               n[5] ? "C" : "", n[6] ? "I" : "");
...
/* open output files ---------------------------------------------------------*/
static int openfile(FILE **ofp, char *files[], const char *file, const rnxopt_t *opt,
                    const nav_t *nav) {
  char path[1024];
  int i;
  ...
  if (event == EVENT_STARTMOVE) {
    ...
    if (norm(p->sta.del, 3) > 0.0) {
      if (!p->sta.deltype) {                  /* enu */
        ...
        trace(2,
              "failed to output RINEX option antenna delta from xyz due to no station position\n");
