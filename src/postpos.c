...
static void procpos(FILE *fp, FILE *fptm, const prcopt_t *popt, const solopt_t *sopt, rtk_t *rtk,
  ...
  solstatic = sopt->solstatic && (popt->mode == PMODE_STATIC || popt->mode == PMODE_STATIC_START ||
...
static int valcomb(const sol_t *solf, const sol_t *solb, double *rbf, double *rbb,
                   const prcopt_t *popt) {
  double dr[3], var[3];
  int i;
  char tstr[40];
...
static int readobsnav(gtime_t ts, gtime_t te, double ti, const char **infile, const int *index,
...
static int antpos(prcopt_t *opt, int rcvno, const obs_t *obs, const nav_t *nav, const sta_t *stas,
                  const char *posfile) {
...
static int openses(const prcopt_t *popt, const solopt_t *sopt, const filopt_t *fopt, nav_t *nav,
...
static int outhead(const char *outfile, const char **infile, int n, const prcopt_t *popt,
                   const solopt_t *sopt) {
  FILE *fp = stdout;
...
static int execses(gtime_t ts, gtime_t te, double ti, const prcopt_t *popt, const solopt_t *sopt,
