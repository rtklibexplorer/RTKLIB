...
static void update_antpos(rtksvr_t *svr, int index) {
  ...
  if (strcmp(svr->rtk.opt.anttype[index], "*") == 0) {
    ...
    tracet(2, "updated antenna index=%d delta to %.4f %.4f %.4f\n", index,
           svr->rtk.opt.antdel[index][0], svr->rtk.opt.antdel[index][1],
           svr->rtk.opt.antdel[index][2]);
  }
  svr->nmsg[index][4]++;
}
 ...
 * return : status (1:ok 0:error)
 *-----------------------------------------------------------------------------*/
extern int rtksvrstart(rtksvr_t *svr, int cycle, int buffsize, int *strs, const char **paths,
...
extern int rtksvrmark(rtksvr_t *svr, const char *name, const char *comment) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (svr->solopt[i].posf == SOLF_STAT) {
      ...
      p += sprintf(p, "$GPTXT,01,01,02,MARK:%s,%s,%.9f,%.9f,%.4f,%d,%s", name, tstr, pos[0] * R2D,
                   pos[1] * R2D, pos[2], svr->rtk.sol.stat, comment);
      for (q = (char *)buff + 1, sum = 0; *q; q++) sum ^= *q; /* check-sum */
      p += sprintf(p, "*%02X\r\n", sum);
    } else {
      p += sprintf(p, "%s MARK: %s,%s,%.9f,%.9f,%.4f,%d,%s\r\n", COMMENTH, name, tstr, pos[0] * R2D,
                   pos[1] * R2D, pos[2], svr->rtk.sol.stat, comment);
    }
