...
/* read SP3 body -------------------------------------------------------------*/
static void readsp3b(FILE *fp, char type, int *sats, int ns, double *bfact, char *tsys, int index,
                     int opt, nav_t *nav) {
  (void)sats;
  peph_t peph;
  gtime_t time;
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    if (strcmp(bias, "OSB") == 0) {
      ...
      if (bias_ix1 == 0) {                    /* this is ref code */
        for (i = 0; i < MAX_CODE_BIASES; i++) /* adjust all other codes by ref code bias */
...
extern int readdcb(const char *file, nav_t *nav, const sta_t *sta) {
  ...
  for (i = 0; i < MAXSAT; i++)
    for (j = 0; j < MAX_CODE_BIAS_FREQS; j++)
...
static int pephpos(gtime_t time, int sat, const nav_t *nav, double *rs, double *dts, double *vare,
                   double *varc) {
 ...
 *-----------------------------------------------------------------------------*/
extern int peph2pos(gtime_t time, int sat, const nav_t *nav, int opt, double *rs, double *dts,
                    double *var) {
  gtime_t time_tt;
