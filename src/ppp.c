...
extern int pppoutstat(rtk_t *rtk, char *buff) {
  ...
  /* receiver position */
  p += sprintf(p, "$POS,%d,%.3f,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n", week, tow, rtk->sol.stat, x[0],
               x[1], x[2], STD(rtk, 0), STD(rtk, 1), STD(rtk, 2));
  ...
  if (rtk->opt.tropopt == TROPOPT_ESTG) {
    ...
    p += sprintf(p, "$TRPG,%d,%.3f,%d,%d,%.5f,%.5f,%.5f,%.5f\n", week, tow, rtk->sol.stat, 1,
                 x[i + 1], x[i + 2], STD(rtk, i + 1), STD(rtk, i + 2));
  }
...
static int sat_yaw(gtime_t time, int sat, const char *type, int opt, const double *rs, double *exs,
                   double *eys) {
...
/* phase windup model --------------------------------------------------------*/
static int model_phw(gtime_t time, int sat, const char *type, int opt, const double *rs,
...
/* tropospheric model ---------------------------------------------------------*/
static int model_trop(gtime_t time, const double *pos, const double *azel, const prcopt_t *opt,
...
/* ionospheric model ---------------------------------------------------------*/
static int model_iono(gtime_t time, const double *pos, const double *azel, const prcopt_t *opt,
...
static int ppp_res(int post, const obsd_t *obs, int n, const double *rs, const double *dts,
                   ...
                   const nav_t *nav, const double *x, rtk_t *rtk, double *v, double *H, double *R,
                   double *azel) {
  ...
  for (i = 0; i < n && i < MAXOBS; i++) {
    ...
    for (j = 0; j < 2 * NF(opt); j++) {
      ...
      /* variance */
      var[nv] =
          varerr(sat, sys, azel[1 + i * 2], rtk->ssat[sat - 1].snr_rover[frq], j, opt, obs + i);
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav) {
  ...
  if (opt->tidecorr) {
    tidedisp(gpst2utc(obs[0].time), rtk->x, opt->tidecorr, &nav->erp, opt->odisp[0], dr);
  }
