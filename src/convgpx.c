...
/* output track --------------------------------------------------------------*/
static void outtrack(FILE *fp, const solbuf_t *solbuf, const char *name, int outalt, int outtime) {
  gtime_t time;
  double pos[3], ep[6];
  int i;
  ...
  for (i = 0; i < solbuf->n; i++) {
    ...
    if (outalt) fprintf(fp, "   <ele>%.4f</ele>\n", pos[2] - (outalt == 2 ? geoidh(pos) : 0.0));
    if (outtime) {
...
/* save gpx file -------------------------------------------------------------*/
static int savegpx(const char *file, const solbuf_t *solbuf, const char *name, int outtrk,
  ...
  if (outpnt) {
    for (i = 0; i < solbuf->n; i++) {
      ecef2pos(solbuf->data[i].rr, pos);
      outpoint(fp, solbuf->data[i].time, pos, solbuf->n == 1 ? name : "", solbuf->data[i].stat,
               outalt, outtime);
    }
  }
