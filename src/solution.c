 ...
 *-----------------------------------------------------------------------------*/
extern int readsolt(const char *files[], int nfile, gtime_t ts, gtime_t te, double tint, int qflag,
                    int mean, solbuf_t *solbuf) {
  FILE *fp;
...
static int decode_solstat(char *buff, solstat_t *stat) {
  ...
  n = sscanf(buff, "$SAT%d%lf%7s%d%lf%lf%lf%lf%d%lf%d%d%d%d%d%d", &week, &tow, id, &frq, &az, &el,
             &resp, &resc, &vsat, &snr, &fix, &slip, &lock, &outc, &slipc, &rejc);
 ...
 * return : status (1:ok,0:no data or error)
 *-----------------------------------------------------------------------------*/
extern int readsolstatt(const char *files[], int nfile, gtime_t ts, gtime_t te, double tint,
...
static int outpos(uint8_t *buff, const char *s, const sol_t *sol, const solopt_t *opt) {
  ...
  if (opt->degf) {
    ...
    p += sprintf(p, "%s%s%4.0f%s%02.0f%s%08.5f%s%4.0f%s%02.0f%s%08.5f", s, sep, dms1[0], sep,
                 dms1[1], sep, dms1[2], sep, dms2[0], sep, dms2[1], sep, dms2[2]);
  } else {
...
static int outenu(uint8_t *buff, const char *s, const sol_t *sol, const double *rb,
                  const solopt_t *opt) {
...
extern int outnmea_gst(uint8_t *buff, const sol_t *sol) {
  ...
  char *p = (char *)buff;
  p += sprintf(p, "$%sGST,%02.0f%02.0f%05.2f,%.3f,%.4f,%.4f,%.3f,%.4f,%.4f,%.4f", NMEA_TID, ep[3],
               ep[4], ep[5], rms, smajor, sminor, angle * R2D, lat_std, lon_std, height_std);
  char sum = 0;
...
extern int outprcopts(uint8_t *buff, const prcopt_t *opt) {
  ...
  const char *s3[] = {"Forward", "Backward", "Combined-Phase Reset", "Combined-No Phase Reset",
                      "",        ""};
  const char *s4[] = {
      "OFF", "Broadcast", "SBAS", "Iono-Free LC", "Estimate TEC", "IONEX TEC", "QZSS Broadcast", "",
      "",    "",          ""};
...
extern int outsolheads(uint8_t *buff, const solopt_t *opt) {
  ...
  if (opt->posf == SOLF_LLH) { /* lat/lon/hgt */
    ...
    if (opt->outvel) {
      p += sprintf(p, "%s%10s%s%10s%s%10s%s%9s%s%8s%s%8s%s%8s%s%8s%s%8s", sep, "vn(m/s)", sep,
                   "ve(m/s)", sep, "vu(m/s)", sep, "sdvn", sep, "sdve", sep, "sdvu", sep, "sdvne",
                   sep, "sdveu", sep, "sdvun");
    }
    ...
    if (opt->outvel) {
      p += sprintf(p, "%s%10s%s%10s%s%10s%s%9s%s%8s%s%8s%s%8s%s%8s%s%8s", sep, "vx(m/s)", sep,
                   "vy(m/s)", sep, "vz(m/s)", sep, "sdvx", sep, "sdvy", sep, "sdvz", sep, "sdvxy",
                   sep, "sdvyz", sep, "sdvzx");
    }
    ...
    p += sprintf(p,
                 ...
                 "sdue(m)", sep, "age(s)", sep, "ratio");
    if (opt->outvel) {
      p += sprintf(p, "%s%10s%s%10s%s%10s%s%9s%s%8s%s%8s%s%8s%s%8s%s%8s", sep, "ve(m/s)", sep,
                   "vn(m/s)", sep, "vu(m/s)", sep, "sdve", sep, "sdvn", sep, "sdvu", sep, "sdven",
                   sep, "sdvnu", sep, "sdvue");
    }
  }
  p += sprintf(p, "\r\n");
