...
/* output track --------------------------------------------------------------*/
static void outtrack(FILE *f, const solbuf_t *solbuf, const char *name, const char *color,
...
/* output point --------------------------------------------------------------*/
static void outpoint(FILE *fp, gtime_t time, const double *pos, const char *label, const char *name,
                     int style, int outalt, int outtime) {
  double ep[6], alt = 0.0;
  char str[256] = "";
  ...
  if (outtime) {
    ...
    sprintf(str, "%04.0f-%02.0f-%02.0fT%02.0f:%02.0f:%05.2fZ", ep[0], ep[1], ep[2], ep[3], ep[4],
            ep[5]);
...
/* save kml file -------------------------------------------------------------*/
static int savekml(const char *file, const solbuf_t *solbuf, const char *name, int tcolor,
...
// Save CSV file ----------------------------------------------------------------
static int savecsv(const char *file, const solbuf_t *solbuf, const char *name, int outalt,
                   int outtime, int outorder) {
