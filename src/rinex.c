...
static void timestr_rnx(char str[32]) {
  ...
  time.sec = 0.0;
  time2epoch(time, ep);
  snprintf(str, 32, "%04.0f%02.0f%02.0f %02.0f%02.0f%02.0f UTC", ep[0], ep[1], ep[2], ep[3], ep[4],
           ep[5]);
}
...
static int sisa_index(double value) {
   ...
   * Available(NAPA).NAPA is an indicator of a potential anomalous signal-in-space[6].Please notice
   * that SISA flag refers to the dual-frequency signal combinations.
   */
...
static int decode_obsdata(FILE *fp, char *buff, double ver, int mask, sigind_t *index,
                          obsd_t *obs) {
  sigind_t *ind;
...
extern void free_rnxctr(rnxctr_t *rnx) {
  ...
  free(rnx->nav.eph);
  rnx->nav.eph = NULL;
  rnx->nav.n = rnx->nav.nmax = 0;
  free(rnx->nav.geph);
  rnx->nav.geph = NULL;
  rnx->nav.ng = rnx->nav.ngmax = 0;
  free(rnx->nav.seph);
  rnx->nav.seph = NULL;
  rnx->nav.ns = rnx->nav.nsmax = 0;
}
/* open RINEX data -------------------------------------------------------------
...
extern int input_rnxctr(rnxctr_t *rnx, FILE *fp) {
  ...
  /* read RINEX OBS data */
  if (rnx->type == 'O') {
    if ((n = readrnxobsb(fp, rnx->opt, rnx->ver, &rnx->tsys, rnx->tobs, &flag, rnx->obs.data,
                         &rnx->sta)) <= 0) {
      rnx->obs.n = 0;
      return n < 0 ? -2 : 0;
    }
...
static void outrnx_phase_shift(FILE *fp, const rnxopt_t *opt, const nav_t *nav) {
  (void)nav;
  static const uint8_t ref_code[RNX_NUMSYS][10] = {
...
static void outrinexevent(FILE *fp, const rnxopt_t *opt, const obsd_t *obs, const double epdiff) {
  ...
  if (opt->rnxver <= 299) { /* ver.2 */
    ...
    fprintf(fp, " %02d %02.0f %02.0f %02.0f %02.0f%11.7f  %d%3d", (int)epe[0] % 100, epe[1], epe[2],
...
extern int outrnxobsb(FILE *fp, const rnxopt_t *opt, const obsd_t *obs, int n, int flag) {
  ...
  for (i = 0; i < ns; i++) {
    ...
    for (int j = 0; j < opt->nobs[m]; j++) {
      ...
      switch (opt->tobs[m][j][0]) {
        ...
        case 'D':
          ...
          break;
        case 'S':
          outrnxobsf(fp, obs[ind[i]].SNR[k], -1, -1);
          break;
      }
    }
    ...
    if (gettracelevel() == 1) {
      trace(1,
            ",%16.2f,%3d,%13.2f,%13.2f,%9.2f,%2.0f,%1d,%3.4f,%13.2f,%13.2f,%9.2f,%2.0f,%1d,%3.4f\n",
            obs[0].time.time + obs[0].time.sec, obs[ind[i]].sat, obs[ind[i]].P[0], obs[ind[i]].L[0],
            obs[ind[i]].D[0], obs[ind[i]].SNR[0], obs[ind[i]].LLI[0], obs[ind[i]].Lstd[0],
            obs[ind[i]].P[1], obs[ind[i]].L[1], obs[ind[i]].D[1], obs[ind[i]].SNR[1],
            obs[ind[i]].LLI[1], obs[ind[i]].Lstd[1]);
    }
#endif
...
extern int outrnxhnavb(FILE *fp, const rnxopt_t *opt, const seph_t *seph) {
  ...
  if (opt->rnxver <= 299) { /* ver.2 */
    fprintf(fp, "%2d %02d %02.0f %02.0f %02.0f %02.0f %04.1f", prn - 100, (int)ep[0] % 100, ep[1],
            ep[2], ep[3], ep[4], ep[5]);
    sep = "   ";
  } else { /* ver.3 */
