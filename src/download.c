...
static int test_file(const char *local) {
  ...
  strcpy(buff, local);
  if ((p = strrchr(buff, '.')) && (!strcmp(p, ".z") || !strcmp(p, ".gz") || !strcmp(p, ".zip") ||
...
static int gen_path(gtime_t time, gtime_t time_p, int seqnos, int seqnoe, const url_t *url,
...
static int gen_paths(gtime_t time, gtime_t time_p, int seqnos, int seqnoe, const url_t *url,
...
static int mkdir_r(const char *dir) {
  ...
  if (CreateDirectory(dir, NULL) || GetLastError() == ERROR_ALREADY_EXISTS) return 1;
...
static int rep_paths(path_t *path, const char *file) {
  ...
  if ((p = strrchr(buff1, '/')))
    ...
    p = buff1;
  if ((q = strrchr(buff2, RTKLIB_FILEPATHSEP)))
    q++;
  else
    q = buff2;
  strcpy(p, file);
...
/* execute download ----------------------------------------------------------*/
static int exec_down(path_t *path, char *remot_p, const char *usr, const char *pwd,
  ...
  if (proto == 0 || proto == 2) {
    sprintf(cmd,
            ...
            env, FTP_CMD, path->remot, usr, pwd, opt, FTP_RETRY, FTP_TIMEOUT, path->local, errfile,
            opt2);
  } else {
    if (*pwd) {
      sprintf(opt + strlen(opt), " --http-user=%s --http-password=%s ", usr, pwd);
    }
    sprintf(cmd, "%s%s %s %s-t %d -T %d -O \"%s\" -o \"%s\"%s\n", env, FTP_CMD, path->remot, opt,
            FTP_RETRY, FTP_TIMEOUT, path->local, errfile, opt2);
  }
...
/* test local file -----------------------------------------------------------*/
static int test_local(gtime_t ts, gtime_t te, double ti, const char *path, const char *sta,
...
/* test local files ----------------------------------------------------------*/
static int test_locals(gtime_t ts, gtime_t te, double ti, const url_t *url, const char **stas,
...
static int print_total(const url_t *url, const char **stas, int nsta, int *nc, int *nt, FILE *fp) {
  int i;
...
extern int dl_exec(gtime_t ts, gtime_t te, double ti, int seqnos, int seqnoe, const url_t *urls,
  ...
  sprintf(msg, "OK=%d No_File=%d Skip=%d Error=%d (Time=%.1f s)", n[0], n[1], n[2], n[3],
          (tickget() - tick) * 0.001);
